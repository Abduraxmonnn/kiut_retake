...
class AdminsListViewSet(viewsets.ModelViewSet):
    """
       A ViewSet for listing student information.
       ...
       The API endpoints for this ViewSet are dynamically generated based on the URL routing.
       Common patterns include:
       - GET api/admins/list/ : List all students
       - GET api/admins/{student_id}/ : Retrieve a student

       For more details on the fields and data formats, refer to the UserListSerializer.
       """
    queryset = User.objects.filter(is_staff=True, is_superuser=True).select_related('univer_group')
    serializer_class = UserListSerializer
    permission_classes = [IsAdminUser]
